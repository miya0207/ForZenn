...
"""
Anthropic Tool Use の完全実装
- スキーマ設計のベストプラクティス
- エラーの適切な伝達
- ツールチェーン（ツールの結果を次のツールへ）
"""
...
GOOD_SCHEMAS = [
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "sort": {
                    "type": "string",
                    "enum": ["new", "popular", "relevant"],
                    "description": "並び順: new=新着, popular=人気, relevant=関連度"
                }
            },
            "required": ["keyword"]
        }
    ...
    {
        "name": "analyze_seo",
        "description": "記事のSEOスコアを分析する。タイトル・本文・キーワードを評価し改善提案を返す。",
        "input_schema": {
            "type": "object",
            "properties": {
                "title": {"type": "string"},
...
def tool_search_articles(keyword: str, limit: int = 5, sort: str = "relevant") -> dict:
    """ダミー実装。実際はDB/API を呼ぶ"""
    return {
        "results": [
            {"id": f"art_{i}", "title": f"{keyword} 入門 Part{i}", "views": 100 * i}
...
def tool_create_article(title: str, content: str, category: str, tags: list = None) -> dict:
    """記事作成。エラーを構造化して返す"""
    from pathlib import Path
    
    if len(title) > 100:
        # ❌ アンチパターン: 例外を投げる → Claudeに伝わらない
        # raise ValueError("タイトルが長すぎます")
        
        # ✅ 正しい: エラーを構造化JSONで返す → Claudeが理解して修正できる
        return {
            "error": "TITLE_TOO_LONG",
            "message": f"タイトルは100文字以内にしてください（現在: {len(title)}文字）",
            "suggestion": "タイトルを短縮してから再度呼び出してください"
        }
    
    path = Path(f"output/drafts/{title[:30].replace('/', '_')}.md")
...
def tool_analyze_seo(title: str, content: str, target_keyword: str) -> dict:
    """SEO分析"""
    score = 0
    suggestions = []
    ...
    if target_keyword in title:
        ...
        suggestions.append(f"タイトルに '{target_keyword}' を含めることを推奨")
    
    keyword_density = content.count(target_keyword) / max(len(content.split()), 1)
    if 0.01 <= keyword_density <= 0.03:
        score += 40
    else:
        suggestions.append(f"キーワード密度を1〜3%に調整（現在: {keyword_density:.1%}）")
    
    if len(content) >= 2000:
        score += 30
    else:
        suggestions.append(f"文字数を2000字以上に（現在: {len(content)}字）")
    
    return {"seo_score": score, "max_score": 100, "suggestions": suggestions}


TOOL_REGISTRY = {
    "search_articles": tool_search_articles,
    "create_article": tool_create_article,
...
class ToolUseAgent:
    ...
    def run(self, user_message: str, max_turns: int = 10) -> str:
        ...
        for turn in range(max_turns):
            ...
            messages.append({
                "role": "assistant",
                "content": response.content  # ← ContentBlock のリストをそのまま追加
            })
            ...
            for block in response.content:
                ...
                print(f"  🔧 ツール呼び出し: {tool_name}({json.dumps(tool_input, ensure_ascii=False)[:80]})")
                
                start = time.time()
                try:
                    fn = TOOL_REGISTRY.get(tool_name)
                    if not fn:
                        result = {"error": "TOOL_NOT_FOUND", "message": f"ツール '{tool_name}' は存在しません"}
                    else:
                    ...
                    result = {
                        ...
                        "required_schema": next(
                            (s for s in GOOD_SCHEMAS if s["name"] == tool_name), None
                        )
                    }
                except Exception as e:
                    result = {
                        ...
                        "message": str(e),
                        "traceback": traceback.format_exc()[-300:]  # 短く切る
                    }
...
本章ではAIエージェントの主要なアーキテクチャパターンを体系的に学びました。
...
| **Multi-Agent** | 大規模・専門分業 | スケールしやすい | 調整コスト高 |
| **RAG** | 知識参照型タスク | ハルシネーション削減 | インデックス管理が必要 |
...
- **記事生成** → ReAct（品質検査ループ）
- **SEO最適化** → 単一責任ツール
- **Discord承認** → Human-in-the-loop（Multi-Agentの変形）

## 次章予告

第9章では、このアーキテクチャを「スケールさせる」技術を学びます。  
asyncioによる並列API呼び出し（3〜5倍高速化）・セマンティックキャッシュ（60%コスト削減）・Haiku/Sonnet自動ルーティングを実装します。
