...
- [ ] **エージェントの状態遷移図（State Machine）を設計書に含めている**
  ...
  > **確認コマンド/設定例**:
  > ```python
  > # Mermaidで状態遷移図を書き起こす（設計書として残す）
  > """
  > stateDiagram-v2
  >     [*] --> Idle
  >     Idle --> Planning : タスク受信
  >     Planning --> Executing : プラン確定
  >     Executing --> Reflecting : ツール呼び出し完了
  >     Reflecting --> Executing : 継続判断
  >     Reflecting --> Done : 完了判断
  >     Executing --> Error : 例外発生
  >     Error --> Idle : リカバリ後
  > """
  > ```
...
- [ ] **プロンプトテンプレートはコードと分離し、バージョン管理している**

  > **なぜ重要か**: プロンプトはエージェントの「ビジネスロジック」。コードに埋め込むと、A/Bテストもレビューも不可能になる。YAMLやMarkdownで管理し、バージョニングすることで品質改善ループを回せる。

---

## チェックリスト活用ガイド

このチェックリストは **設計レビュー時** と **実装完了時** の2回使うことを推奨します。

| タイミング | 目的 | 合格基準 |
|----------|------|---------|
| 設計レビュー時 | アーキテクチャの問題を早期発見 | 必須項目（🔴）が全てチェック済み |
| 実装完了時 | 本番投入前の最終確認 | 全項目チェック済み（🟡含む） |

> **注記:** 🔴 = 必須（スキップ不可）/ 🟡 = 推奨（理由があれば省略可能）
